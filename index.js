...
const prom_pushgateway = process.env.PROM_PUSHGATEWAY_HOSTNAME || 'prom-pushgateway'
const interval = +(process.env.WEAVE_SCRAPE_INTERVAL || '15');
const instance = `${(process.env.INSTANCE_NAME || execSync('hostname'))}`.trim();

console.log(`Starting exporter with: push_gateway_ip: ${prom_pushgateway}, hostname: ${instance}, interval: ${interval}, metrics host: ${process.env.WEAVE_METRICS_HOST}`);
...
setInterval(() => {
  (async () => {
    try {
      ...
      await axios.post('http://' + prom_pushgateway + '/metrics/job/weave_exporter/instance/' + instance, metrics, {timeout: 5000});
